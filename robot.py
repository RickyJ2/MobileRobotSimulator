...
class Robot:
    def __init__(self, startPos: Point, width, scale, HexagonSize):
        ...
        self.rect = self.rotated.get_rect(center=((startPos.x + HexagonSize) * scale, (startPos.y + HexagonSize) * scale))
    ...
    def move(self):
        ...
        self.currentPose.point.x += v * math.cos(math.radians(self.currentPose.orientation))
        self.currentPose.point.y += v * math.sin(math.radians(self.currentPose.orientation))
        self.currentPose.orientation += omega
        self.rotated = pygame.transform.rotozoom(self.body, 360 - self.currentPose.orientation,1)
        self.rect = self.rotated.get_rect(center=((self.currentPose.point.x + self.HexagonSize) * self.scale, (self.currentPose.point.y + self.HexagonSize) * self.scale))
